...
const CartographieAvancee = () => {
  ...
  const handleServiceUpdate = (serviceId, field, value) => {
    ...
    const updatedServicesWithTotals = updatedServices.map(service => {
      if (service.id === serviceId) {
        ...
        const categoriesEmployees = Object.values(service.categories).reduce(
          (sum, cat) => sum + (cat.employees || 0), 0
        );
        const categoriesSyndiques = Object.values(service.categories).reduce(
          (sum, cat) => sum + (cat.syndiques || 0), 0
        );
  ...
  const handleSaveLocal = async () => {
    return storageService.saveLocally('cartographieAvancee', servicesData);
  };

  // Fonction pour la sauvegarde sur le serveur
  const handleSaveServer = async () => {
    return storageService.saveToServer('cartographieAvancee', servicesData);
  };
